...
function isUniversidade(cat: { name: string; slug: string }) {
  const n = cat.name.toLowerCase();
  const s = cat.slug.toLowerCase();
  return n.includes("universidade") || s.includes("universidade");
}
    ...
    <div className="bg-white rounded-2xl border border-gray-100 overflow-hidden hover:border-gray-200 hover:shadow-md transition-all group">
      ...
      <button onClick={onOpen} className="w-full text-left focus:outline-none">
        ...
        <div className="px-4 pt-2.5 pb-4">
          <p className="font-semibold text-gray-900 text-sm leading-snug line-clamp-2 group-hover:text-indigo-600 transition-colors">
            {material.title}
          </p>
          {material.description && (
            <p className="text-xs text-gray-500 mt-1 line-clamp-2">{material.description}</p>
          )}
          <p className="text-xs text-indigo-600 font-medium mt-2 flex items-center gap-0.5">
            Assistir <ChevronRight className="h-3 w-3" />
          </p>
        </div>
      </button>
    </div>
...
export default function UniversidadePage() {
  ...
  useEffect(() => {
    fetch("/api/materials")
      .then((r) => r.json())
      .then((mats: Material[]) => {
        setMaterials(Array.isArray(mats) ? mats.filter((m) => isUniversidade(m.category)) : []);
        setLoading(false);
      });
  }, []);
  ...
  return (
    <div className="p-4 sm:p-6">
      ...
      <div className="flex items-center gap-3 mb-2">
        ...
        <div>
          <h1 className="text-xl font-bold text-gray-900">Universidade</h1>
          <p className="text-sm text-gray-500">
            {loading
              ? "Carregando..."
              : `${filtered.length} de ${materials.length} tutorial${materials.length !== 1 ? "is" : ""}`}
          </p>
        </div>
      </div>

      <p className="text-sm text-gray-500 mb-5 max-w-2xl">
        Vídeos e tutoriais para aprender a usar a plataforma e tirar o máximo de cada recurso.
      </p>
      ...
      <div className="relative mb-5">
        ...
        <input
          type="text"
          placeholder="Pesquisar tutorial por nome..."
          value={search}
          onChange={(e) => setSearch(e.target.value)}
          className="w-full pl-9 pr-9 py-2.5 rounded-xl border border-gray-200 text-sm text-gray-700 placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-transparent bg-white transition"
        />
        {search && (
          <button
            onClick={() => setSearch("")}
      ...
      {loading ? (
        ...
        <div className="text-center py-16">
          ...
          <p className="text-gray-500 mt-4 font-medium">Nenhum tutorial disponível ainda.</p>
          <p className="text-sm text-gray-400 mt-1">
            Os vídeos de treinamento da plataforma aparecerão aqui quando publicados.
          </p>
        </div>
      ) : filtered.length === 0 ? (
        <div className="text-center py-16">
