...
import { GraduationCap, Plus, Trash2, Loader2, FileText, Play, X, Search, Eye, EyeOff } from "lucide-react";
...
function isUniversidadeCategory(cat: { name: string; slug: string }) {
  return (
    cat.name.toLowerCase().includes("universidade") ||
    cat.slug.includes("universidade")
  );
}

export default function AdminUniversidadePage() {
  ...
  const fetchData = useCallback(async () => {
    ...
    const uniCat = Array.isArray(cats) ? cats.find(isUniversidadeCategory) : null;
    setCategory(uniCat ?? null);

    if (Array.isArray(mats)) {
      setMaterials(mats.filter((m: Material) => isUniversidadeCategory(m.category)));
    }
    setLoading(false);
  }, []);
  ...
  // Garante que a categoria Universidade exista antes de salvar
  async function ensureCategory(): Promise<Category | null> {
    ...
    const res = await fetch("/api/categories", {
      ...
      body: JSON.stringify({
        name: "Universidade",
        description: "Tutoriais e vídeos de treinamento da plataforma",
        icon: "🎓",
      }),
    });
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (!cat) {
      setError("Não foi possível criar a categoria Universidade");
      setSaving(false);
      return;
    }
  ...
  async function handleDelete(material: Material) {
    if (!confirm(`Excluir o tutorial "${material.title}"?`)) return;
  ...
  async function toggleVisibility(material: Material) {
    ...
    if (!res.ok) {
      ...
      alert("Erro ao alterar a visibilidade do tutorial");
    }
  }
    ...
    <div className="p-4 sm:p-6 md:p-8">
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-xl md:text-2xl font-bold text-gray-900 flex items-center gap-2">
            <GraduationCap className="h-6 w-6 text-indigo-500" /> Universidade
          </h1>
          <p className="text-gray-500 text-sm mt-1">
            Publique vídeos e tutoriais de uso da plataforma para a rede
          </p>
        </div>
        <button
          onClick={openCreate}
          className="flex items-center gap-2 px-3 sm:px-4 py-2 bg-indigo-600 hover:bg-indigo-700 text-white font-medium rounded-lg transition text-sm"
        >
          <Plus className="h-4 w-4" />
          <span className="hidden sm:inline">Novo Tutorial</span>
      ...
      <div className="relative max-w-sm mb-6">
        ...
        <input
          ...
          className="w-full pl-9 pr-3 py-2 bg-white border border-gray-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500"
        />
      </div>
            ...
            >
              <div className="relative">
                {material.fileType === "IMAGE" && material.fileUrl ? (
                  ...
                  <div className={`aspect-video bg-indigo-50 flex flex-col items-center justify-center gap-2 text-indigo-500 ${material.published ? "" : "opacity-50 grayscale"}`}>
                  ...
                  <div className="flex items-center gap-1">
                    {material.fileUrl && (
                      <a
                        ...
                        className="px-2.5 py-1 text-xs font-medium text-indigo-700 bg-indigo-50 hover:bg-indigo-100 rounded-lg transition"
                      >
                        Abrir
                      </a>
                    )}
        ...
        <div className="fixed inset-0 bg-black/50 flex items-end sm:items-center justify-center z-50">
          <div className="bg-white rounded-t-2xl sm:rounded-2xl shadow-xl w-full sm:max-w-md p-6 max-h-[95vh] overflow-y-auto">
            <div className="flex items-center justify-between mb-5">
              <h2 className="text-xl font-bold text-gray-900">Novo Tutorial</h2>
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              <div>
                ...
                <input
                  ...
                  placeholder="Como enviar uma solicitação"
                />
              </div>
              <div>
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-2">Vídeo ou arquivo *</label>
                {pendingFile ? (
                  ...
                  <button
                    ...
                    disabled={uploading}
                    className="flex items-center gap-2 px-4 py-2.5 border border-dashed border-gray-300 rounded-xl text-sm text-gray-500 hover:border-indigo-400 hover:text-indigo-600 transition w-full justify-center"
                  >
                    {uploading ? <Loader2 className="h-4 w-4 animate-spin" /> : <Plus className="h-4 w-4" />}
                    {uploading ? "Enviando arquivo…" : "Anexar vídeo (ou PDF)"}
                  </button>
                )}
                <input
                  ref={fileInputRef}
                ...
                />
                <p className="text-xs text-gray-400 mt-1.5">Vídeos até 2GB (MP4, MOV, WEBM…).</p>
              </div>
              ...
              <div className="flex gap-3 pt-2">
                ...
                <button
                  ...
                  className="flex-1 py-2.5 bg-indigo-600 hover:bg-indigo-700 disabled:opacity-50 text-white font-medium rounded-lg transition text-sm"
                >
                  {saving ? "Salvando..." : "Publicar Tutorial"}
                </button>
              </div>
            </form>
          </div>
